...
- **`ui.contribution_reminder`** — set to `false` to remove the "help grow the
  dataset" banner shown at the top of the generated `fiverr-catalog.html`
  (on by default; it's what the end user sees when they open the catalog).
